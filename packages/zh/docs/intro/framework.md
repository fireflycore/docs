...
适用于中小型规模集群、IDC 或暂未进入 K8s 的环境。核心特点是业务服务通过 `go-consul/agent` 接入本机 `sidecar-agent`，由 CoreDNS、Envoy、Consul、api-gateway 和 authz 承接治理能力。
...
graph TD
    ...
    NodeEnvoy --> ServiceA
    NodeEnvoy --> ServiceB
    ServiceA -->|Service DNS| NodeDNS
    NodeDNS -->|mesh VIP| NodeEnvoy
```
...
- `sidecar-agent`：本机控制面，负责注册、摘流、注销、watch/replay、route document、东西向 xDS 和健康探测。
- `CoreDNS`：节点级 DNS 基础设施，解析 `*.svc.cluster.local` 到本机 mesh VIP；DNS 不负责透明代理。
