...
- **开箱即用**：利用 `sidecar-agent`、`api-gateway`、Envoy 和 `authz`，逐步获得注册发现、入口路由、鉴权和基础流量治理能力。
...
- **初创项目**：推荐从**裸机运行时模式**起步。通过 `go-layout + sidecar-agent + Consul + Envoy` 快速搭建具备注册、配置、路由和鉴权接入能力的微服务集群。
